...
class MB_SpeechComponent(nn.Module):
    ...
    def forward(self, x):
        ...
        if __name__ == "__main__":
            print('after encoder size', w.size())
            print('after encoder lenght', len(x))
...
class MS_SL2_split_channelwise_model(nn.Module):
    def __init__(self,
        ...
        # self.wList = th.tensor([0.5+0.001, 0.5-0.001,0.4-0.001,0.4+0.001])
        # self.wList = self.wList.type(th.int64)
        # print("integer weight", self.wList)
    ...
    def forward(self, x):
        ...
        for Slice in range(self.slice):
            ...
            for i in range (len(self.wList)): 
                #if a==0:  
                print("Weight value1", self.wList[i])
                # total_connection = th.sigmoid(self.wList[i] + self.PReLU(skip_connection * self.wList[i]))
                total_connection = skip_connection * self.wList[i]
...
def foo_conv_tas_net():
    ...
    print("ConvTasNet #param: {:.2f}".format(param.shape(nnet)))
    x = nnet(x)
    s1 = x[0]
    print(s1.shape)    
# ***********************************************    
